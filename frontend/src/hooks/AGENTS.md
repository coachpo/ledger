...
| Tool catalog reads | `use-workflow-packages.ts` | extension-filtered read-only server-declared tool metadata for package capability profiles |
...
- Package-first platform hooks invalidate `queryKeys.platform.*` namespaces and keep route-specific polling, extension-state filtering, or mutation policy inside the hooks layer; model-connection connection tests also invalidate persisted last-test metadata.
...
- Do not bypass `use-extensions.ts` for extension state or duplicate route/tool visibility filtering in pages.
