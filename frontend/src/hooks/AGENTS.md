...
## OVERVIEW
`src/hooks/` wraps the `src/lib/api/*.ts` modules with TanStack Query hooks for portfolios, balances, positions, trading operations, market data, templates, reports, orchestration, runtime, Studio, Tryout, and one small UI debounce helper.
...
| Runtime/Studio/Tryout tests | `use-runtime.test.ts`, `use-studio.test.ts`, `use-tryouts.test.ts` | cache policy and v2 contract coverage |
...
- Do not reimplement runtime or Tryout cache invalidation in pages when `use-runtime.ts`, `use-studio.ts`, and `use-tryouts.ts` already model those transitions.
- Do not move route-local UI state into this layer just because a page is busy.
- Do not duplicate orchestration, Studio, or Tryout invalidation logic in page components.
...
- Orchestration hooks follow the same split: cache policy and API wiring live here, while forms and navigation stay in the route family.
