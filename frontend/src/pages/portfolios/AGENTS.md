...
`src/pages/portfolios/` owns the live portfolio inventory and workspace detail routes. The list page handles create/edit/delete entry points, while the detail page composes balances, positions, trades, quotes, and derived portfolio metrics inside one routed workspace.

## WHERE TO LOOK

| Task                    | Location                                                                                                                                | Notes                                                                              |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Portfolio inventory     | `list.tsx`                                                                                                                              | newest-updated ordering, modal create/edit, delete confirmation, compact row cards |
...
- `list.tsx` owns route-level modal state and navigation, but row presentation stays in `ResourceRowCard` and shared dialogs.
...
- Portfolio row navigation must remain an explicit link, while create, edit, delete, trade, and dialog controls remain buttons or form controls.
...
- Do not push section-specific mutation logic down into generic card/table helpers.
