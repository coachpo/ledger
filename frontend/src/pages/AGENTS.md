...
| Memory routes                | `memory/AGENTS.md`, `../hooks/use-memory.ts`, `../lib/api/memory.ts`                    | `/memory` trusted operator admin inventory plus `/memory/:memoryId` detail, revision, event, and status flows with scoped runtime helpers kept separate |
