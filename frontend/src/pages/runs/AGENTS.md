...
## OVERVIEW
`src/pages/runs/` contains the routed run inventory and run detail views. The list page acts as a polling monitor, and the detail page exposes progress, usage, rerun, and visible `Start new run from Step N` step-start flows, trace linkage, and per-agent accordion drilldowns.
...
| Run detail | `detail.tsx` | progress cards, rerun and visible `Start new run from Step N` step-start actions, trace linkage, final output, and per-agent accordion |
...
- Keep the browser step-start wording as `Start new run from Step N`; keep API, route, query, and type contract names in their existing step replay shape.
