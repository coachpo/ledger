...
## OVERVIEW
`src/pages/scheduled-tasks/` owns the package-first automation route family for recurring Workflow Package runs. The list route monitors saved schedules, the create route authors one package/workflow schedule, and the detail route is the console for recurrence, scheduled input previews, run-now, archive, fire history, and linked run evidence.
...
|-- detail.tsx       # full-height console for edit, preview, fire history, archive
|-- list.test.tsx    # inventory states, filters, links, actions
`-- detail.test.tsx  # detail tabs, preview/run-now/archive, read-only states
```

## WHERE TO LOOK
| Task | Location | Notes |
|---|---|---|
| Schedule inventory | `list.tsx` | active/paused/archived filters, local search/sort, explicit open/archive/run-now actions |
| Schedule creation | `editor.tsx` | creates one schedule for a saved package id and workflow key, with preview before save |
| Schedule console | `detail.tsx` | recurrence editor, JSON template/vars editor, preview, run-now, archive, fire history, latest run links |
| Schedule hooks | `../../hooks/use-scheduled-tasks.ts` | list/detail/fire queries, create/update/archive/preview/run-now mutations, linked run invalidation |
| Wire/API contracts | `../../lib/api/schedules.ts`, `../../lib/types/schedule.ts` | `/api/schedules` helpers plus recurrence, fire, preview, and run-now payloads |
...
- Archive preserves fire/run history, clears future scheduling, and makes the detail route read-only. Duplicate/edit through a new schedule instead of mutating archived records.
...
- Do not edit or preview archived schedules as if they were mutable.
