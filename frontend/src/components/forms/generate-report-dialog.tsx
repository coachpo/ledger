...
export function GenerateReportDialog({
  ...
  const resetKey = `${props.defaultTemplateId ?? ""}:${JSON.stringify(
    props.initialInputs ?? {},
  )}`;

  return (
    <Dialog open={open} onOpenChange={props.onOpenChange}>
      {open ? <GenerateReportDialogContent key={resetKey} {...props} /> : null}
    </Dialog>
  );
}

function GenerateReportDialogContent({
  defaultTemplateId,
  ...
  onOpenChange,
  templateOptions,
}: Omit<GenerateReportDialogProps, "open">) {
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Use key/value pairs like `ticker`, `analysis_tag`, or `review_type`
            when the selected template is parameterized.
          </p>
