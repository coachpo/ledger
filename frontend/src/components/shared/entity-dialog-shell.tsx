...
export function EntityDialogShell({
  ...
  return (
    <DialogContent
      className={cn(
        "flex max-h-[calc(100dvh-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[var(--ui-layout-dialog-width)]",
        className,
      )}
    >
      ...
      <Separator />
      <div
        className="min-h-0 flex-1 overflow-auto overscroll-contain bg-ui-surface px-5 py-4 sm:px-6"
        data-slot="entity-dialog-body"
      >
        {children}
      </div>
      <Separator />
      <DialogFooter className="shrink-0 bg-ui-surface-elevated px-5 pb-5 pt-4 sm:px-6 sm:pb-6">
        {footer}
      </DialogFooter>
    </DialogContent>
  );
