...
const badgeVariantByTone: Record<ResourceStatusTone, "default" | "secondary" | "outline" | "destructive"> = {
  neutral: "outline",
  success: "secondary",
  warning: "outline",
  danger: "destructive",
...
export function ResourceStatusStrip({
  ...
  if (items.length === 0) {
    return <div className={cn("rounded-lg border bg-muted/30 px-3 py-2 text-xs text-muted-foreground", className)}>{emptyLabel}</div>;
  }
  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <div className="flex min-w-0 items-center gap-2" key={`${item.label}-${index}`} role="listitem">
          {index > 0 ? <Separator className="hidden h-4 sm:block" orientation="vertical" /> : null}
          <Badge data-tone={item.tone ?? "neutral"} variant={badgeVariantByTone[item.tone ?? "neutral"]}>
            {item.label}
          </Badge>
          {item.value || item.value === 0 ? <span className="min-w-0 truncate font-medium">{item.value}</span> : null}
          {item.description ? <span className="min-w-0 truncate text-muted-foreground">{item.description}</span> : null}
        </div>
      ))}
    </div>
  );
