...
- Document scoped runtime `/api/memory` plus `signaldeck.core.memory.lookup` and `signaldeck.core.memory.write` separately from trusted local operator `/api/memory/admin/entries*` and `/memory` admin management over canonical workflow memory; keep runtime shared namespace grants server-derived and keep historical agent reports plus `signaldeck.finance.reports.lookup` under finance/report ownership.
- Document `visible_to_workflow` internally and `visibleToWorkflow` externally as the live memory runtime-eligibility contract. Runtime lookup is scoped and workflow-visible-only, runtime writes default hidden, admin create defaults visible, and admin list sees all entries by default with optional `visibleToWorkflow=true|false` filtering. Treat `approved`, `pending`, and `archived` only as historical startup-repair inputs, not live Memory Admin states.
