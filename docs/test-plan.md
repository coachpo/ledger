...
| Memory                       | Core schemas, write/reuse/supersede semantics, scoped lookup fallback, namespace grants, conflict handling, runtime tools, trusted operator admin APIs, `/memory` admin behavior, all-status admin visibility, scope/status/provenance rules, run memory events, and report-domain separation. |
