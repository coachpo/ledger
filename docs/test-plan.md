...
> Status: Live automated-coverage reference for branch `main` at `1e43bf7`.
...
Backend coverage must prove preserved `/api/v1` finance CRUD, template/report behavior, package-first platform contracts, Scheduled Tasks API and materialization semantics, backend-owned compatibility resolution, Model Gateway behavior, native runtime tools, Digital Oracle phase-1 finance tools, typed tool-failure taxonomy, bounded `toolCallRetries`, distinct live-execution `providerRetries`, memory services, explicit-private-scope `/api/memory`, scheduler semantics, run provenance, and removed-route guarantees.
