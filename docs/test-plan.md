...
| Scheduled Tasks              | `/api/schedules` CRUD, list filters, hard-delete 204 and archive-route absence, direct schedule-owned run/fire deletion, startup archived-row purge, structured recurrence, timezone and DST cases, overlap and misfire policy outcomes, input placeholder rendering, preview validation without persistence, run-now idempotency, fire history, and linked run provenance. |
...
Scheduled Tasks final-doc verification also checks live docs and route guidance for archive-absence drift, plus the broad build and backend suite:
