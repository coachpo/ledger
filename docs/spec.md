...
SignalDeck memory is platform-core infrastructure for Workflow Package runs in a trusted single-user local/private deployment. Runtime tools are `signaldeck.memory.write` and `signaldeck.memory.lookup`; exact-id `signaldeck.memory.get` is deferred. `/api/memory` remains the scoped runtime-support API, while `/api/memory/admin/entries*` and browser `/memory` form a trusted local operator control plane over canonical workflow memory across packages. These are not finance routes, public multi-user CRUD, or authorization surfaces.

Runtime memory writes use neutral `kind`, `summary`, `content`, `subjectRefs`, `attributes`, canonical scope, status, revision, and trusted workflow provenance. Writes create immutable revisions or reuse an exact duplicate active revision. Shared-scope conflicts return retryable `memory_revision_conflict`.

Memory lookup is scoped. Runtime selectors may use scope, subject refs, kind, status, tags, query, limit, offset, and max characters. When runtime-tool selectors are omitted, the server falls back to current run/package/agent context instead of running unscoped global search. Runtime lookup defaults to resolved entries; pending and expired entries remain admin-visible but are not returned to runtime lookup unless a future contract intentionally changes status selection.
...
`/api/memory` accepts body-carried runtime context and exposes bounded runtime workflows: list scoped memory, load detail, list revisions, list events, resolve, and reflect. Detail, revision, event, and action requests authorize the stored memory scope before returning data or mutating status. These scoped paths do not provide admin-style all-package reads.

`/api/memory/admin/entries*` is the trusted local operator API for list/detail/history/create/revise/status over canonical memory. Admin lists default to all statuses with `limit=50`, max `200`, and `sort=updatedAtDesc`; optional package, workflow, agent, run, scope, kind, status, and query filters narrow the operator-managed corpus. Default all-status visibility is intended operator behavior, not a package-private browser gate.

Admin create, revise, and status changes write the canonical memory tables with explicit scope, status, provenance, immutable revisions, and append-only events. Operator-authored writes use `createdByType=operator`, actor `local-instance-operator`, channel `memory_admin`, and event kinds `operator_created`, `operator_revised`, and `operator_status_changed`. A resolved admin entry whose scope and namespace grants match a future Workflow Package run can affect `signaldeck.memory.lookup`; pending or expired entries remain visible to the admin control plane only.

The browser `/memory` route is the trusted local operator/admin view. It queries the admin list immediately, may show cross-package and mixed-scope rows, supports optional narrowing filters, and renders canonical memory detail, revisions, events, provenance, and admin write/status controls. It does not require choosing a package before loading, and it must not add browser-authored namespace declarations, namespace grants, destructive deletion actions, vector activation/search, or report-history promotion.
