...
Package-local agent prompts own methodology. The Digital Oracle researcher package keeps research policy in package-local agent `systemPrompt` text, grants the local `digital_oracle_phase1_tools` capability profile, reuses Finance-owned history/OHLCV grants for price-history obligations, and reserves `demo/digital_oracle_researcher.yaml` as the final proven artifact path. The TradingAgents advisory demo grants `signaldeck.digital_oracle.prediction_markets.lookup` through a package-local capability profile for prediction-market catalyst research while keeping indicators, fundamentals, news, social sentiment, positions, and reports on Finance-owned tool keys. This is package data, not a global skill or platform orchestration surface.
