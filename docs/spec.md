...
Runtime memory writes use the lean model-facing `signaldeck.core.memory.write` contract: neutral `kind`, `summary`, `content`, `subjectRefs`, canonical scope, idempotency key, and trusted workflow provenance. Runtime writes do not accept arbitrary attributes, runtime tags, public audit links, report identity, or caller-authored namespace grants. The database and internal services use `visible_to_workflow`; external API and frontend payloads use `visibleToWorkflow`. Runtime writes default to hidden with `visibleToWorkflow=false`, create immutable revisions or reuse an exact duplicate active revision, and return retryable `memory_revision_conflict` for shared-scope conflicts.

Memory lookup is scoped and workflow-visible-only. Runtime selectors may use scope, subject refs, kind, query, limit, offset, and max characters. Runtime lookup does not accept tags, arbitrary attributes, report selectors, chunk selectors, embedding selectors, or vector-search knobs. When runtime-tool selectors are omitted, the server falls back to current run/package/agent context instead of running unscoped global search. Runtime lookup returns only `visible_to_workflow=true` entries that match scope and grant rules, and model-visible lookup output omits visibility.

Shared namespaces use `namespace` scope with keys shaped as `{ownerPackageKey}/{namespaceKey}`. Owner packages read and write their declared namespaces, while non-owner packages need explicit read or write grants declared in package/runtime data and enforced by backend services. Wildcards, ownerless namespaces, browser-authored grants, global memory search, and cross-package private writes fail closed.

`/api/memory` accepts body-carried runtime context and exposes bounded runtime workflows: list scoped memory, load detail, resolve, and reflect. Detail and action requests authorize the stored memory scope before returning data or recording scoped actions. These scoped paths do not provide admin-style all-package reads, scoped revision routes, scoped event routes, or scoped history feeds.
...
Admin create, revise, and workflow-visibility changes write the canonical memory tables with explicit scope, `visible_to_workflow`, subject refs, operator provenance, immutable revisions, and append-only typed events. Admin create defaults visible with `visibleToWorkflow=true`. Operator-authored writes use `createdByType=operator`, actor `local-instance-operator`, channel `memory_admin`, and event kinds `operator_created`, `operator_revised`, and `operator_visibility_changed`; review facts live in typed `run_memory_events` snapshots. A visible admin entry whose scope and namespace grants match a future Workflow Package run can affect `signaldeck.core.memory.lookup`; hidden entries remain visible to the admin control plane only.

Admin hard delete is explicit and single-entry only: `DELETE /api/memory/admin/entries/{memoryId}` returns `204 No Content` for an existing entry and uses the existing memory not-found error for missing or repeated deletes. The delete removes the entry plus dependent revisions through existing cascades. Existing `run_memory_events` survive as run evidence with preserved `memory_id` and `revision_id` snapshot strings, nullable numeric memory foreign keys, and no delete event. Runtime delete, bulk delete, global delete, tombstones, undo/recycle bin, delete reasons, and run/package ownership cascades are non-goals.
...
The browser `/memory` route is the trusted local operator/admin list view. It queries the admin list immediately, may show cross-package and mixed-scope rows, supports optional narrowing filters, and links each row to `/memory/:memoryId`. The routed detail view renders canonical memory detail, admin-only revisions, admin-only events, compact provenance, revision creation, admin workflow visibility controls, and one-entry delete for one entry. It does not require choosing a package before loading, and it must not add raw Attributes JSON editing, browser-authored namespace declarations, namespace grants, bulk delete, runtime/global delete, tombstones, undo/recycle-bin behavior, delete reasons, vector activation/search, chunk or embedding controls, core-memory audit links, or report-history promotion.

Model-visible runtime lookup outputs may include memory/revision ids, kind, summary, content, subject refs, scope, compact provenance, and warnings. Runtime lookup output omits `visibleToWorkflow`; runtime write output includes `visibleToWorkflow`. Model-visible outputs must not expose arbitrary attributes, runtime tags, report identity, raw markdown, download URLs, audit links, chunks, embeddings, or vector scores. API and UI memory projections return canonical memory only and do not include finance-owned report-history rows. Run memory artifacts may keep their own non-runtime report or audit evidence when captured as run evidence, but core runtime memory does not expose that evidence as lookup fields.

Chunk, embedding, and vector persistence are not live shipped memory behavior. Startup repair may drop obsolete memory chunk and embedding tables as removal coverage, but live runtime, admin, API, UI, and tool contracts must not document those structures as deferred scaffolding or selectable retrieval modes.
