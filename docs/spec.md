...
`GET /api/schedules/{scheduleId}/fires` returns paged fire history with status, reason, local scheduled fields, rendered parameters, skip or error details, and linked run id while the schedule exists. `DELETE /api/schedules/{scheduleId}` returns 204 with no body, removes the schedule and its fire rows, and deletes direct schedule-owned runs through the run deletion path. Rerun and fork descendants are ordinary run lineage and are not deleted just because their ancestor came from a schedule. Startup schema repair purges legacy archived schedule rows through this same hard-delete service path. OpenAPI and regression tests assert the 204 delete operation and the absence of the old archive route.
