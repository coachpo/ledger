...
> Status: Live API reference for branch `main` at `f9ae90d`.
...
| Resource | Routes |
|---|---|
| Workflow packages | `GET/POST /api/workflow-packages`, `GET/PATCH/DELETE /api/workflow-packages/{packageId}`, `GET /api/workflow-packages/{packageId}/manifest`, `POST /api/workflow-packages/validate-manifest`, `POST /api/workflow-packages/import`. Live package reads and writes do not include `status`; stale `status` filters or update fields return `422`. Package persistence stores dependency keys as artifact references, while readiness endpoints evaluate those refs against live model connections, extension state, and package secret bindings. Deleting a package deletes its owned runs. |
| Package secret bindings | `GET /api/workflow-packages/{packageId}/secret-bindings`, `PUT/DELETE /api/workflow-packages/{packageId}/secret-bindings/{key}`. Delete removes the live binding even when current package artifacts still reference its key; later readiness/runtime checks surface the missing value. |
| Package exports and launches | `GET /api/workflow-packages/{packageId}/export`, `POST /api/workflow-packages/{packageId}/preflight`, `GET /api/workflow-packages/{packageId}/launch`, `POST /api/workflow-packages/{packageId}/launches`. Launch creates a durable queued run after current readiness passes. |
| Model connections | `GET/POST /api/model-connections`, `GET/PATCH/DELETE /api/model-connections/{connectionId}`, `POST /api/model-connections/{connectionId}/connection-test`, `POST /api/model-connections/{connectionId}/capability-probe`. Delete is a live-environment mutation: packages keep referenced keys as artifacts, and later readiness/runtime checks report missing or unusable connections. |
...
Model Connection create/update/read payloads use `protocolProfile` as the live protocol selector, with allowed values `openai_chat_completions` and `openai_responses`. Reads also include `connectionKind`, `baseUrl`, `modelId`, optional `reasoningEffort`, `capabilities`, `outputStrategyPolicy`, `parallelToolCallsPolicy`, `reasoningPolicy`, `streamingPolicy`, `lastProbedAt`, `probeCacheTtlSeconds`, timeout, and reachability-test metadata. Capability statuses are `supported`, `unsupported`, `unknown`, and `notApplicable`; each capability state may include `detail` and `lastProbedAt`. `apiStyle` is historical compatibility metadata derived from `protocolProfile`, not the primary live selector.

`POST /api/model-connections/{connectionId}/connection-test` is reachability-only and returns `{modelConnectionId, ok, message, lastTestedAt}`. `POST /api/model-connections/{connectionId}/capability-probe` accepts optional `{capabilityKeys, refresh}` and returns `{modelConnectionId, requestedCapabilityKeys, cached, lastProbedAt, probeCacheTtlSeconds, capabilities}`. Probe requests normalize capability keys to the public capability names and reject unknown or duplicate keys.

Run list and detail reads include backend-owned `progress` and nullable `queue` objects. `progress` is derived from persisted agent and operation invocation statuses and serializes as `unit`, `terminalCount`, `totalCount`, and `percent`, with terminal runs reporting `percent: 100`. `queue` explains backend-derived queued-state reasons such as serial package-lane blocking or awaiting worker capacity; status remains a lifecycle value only.

Run package provenance exposes sanitized `resolvedModelConnections` as launch-time effective runtime profile evidence. Each resolved connection includes key, name, `connectionKind`, `protocolProfile`, base URL, model id, optional reasoning effort, capabilities, output/tool/reasoning/streaming policies, probe cache TTL, derived historical `apiStyle`, timeout, and `hasApiKey`. It never includes raw API keys, headers, or provider payloads.

Rerun drafts expose root launch `parameters`, historical package provenance, and top-level current readiness fields. Rerun creates are the only run-descendant write flow that edits root parameters and they create queued descendants. Fork drafts expose the selected source agent invocation's persisted input by `sourceInvocationId`, historical provenance, and top-level current readiness fields. Fork creates accept `sourceInvocationId` and full replacement `invocationInput`, preserve the source run input, copy upstream context, persist `run_forks`, and resume execution at `resumeStepIndex`. Historical step replay fields can still appear in read payloads for old run lineage, but `/step-replay-draft` and `/step-replays` are not live write routes.
...
- Model Connections are global live bindings; package manifests store model connection keys, not provider credentials. `protocolProfile`, capability states, policies, and probe cache metadata decide current model compatibility. Deleting a connection affects current readiness and subsequent execution, not historical run snapshots.
...
- `409` for uniqueness conflicts such as duplicate slugs/keys/names, and for retryable shared-memory mutation conflicts such as `memory_revision_conflict`.
