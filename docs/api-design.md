...
| Model connections | `GET/POST /api/model-connections`, `GET/PATCH/DELETE /api/model-connections/{connectionId}`, `POST /api/model-connections/{connectionId}/connection-test`. Delete is a live-environment mutation: packages keep referenced keys as artifacts, and later readiness/runtime checks report missing or unusable connections. |
...
- Model Connections are global live bindings; package manifests store model connection keys, not provider credentials. Deleting a connection affects current readiness and subsequent execution, not historical run snapshots.
