...
> Status: Live data-model reference for branch `main` at `987686e`.
...
| `runs` | global persisted package execution input/output, status, totals, optional Logfire trace ids, rerun/replay metadata, package provenance, and launch snapshots |
...
- Runs store package id, package key, package version, package hash, workflow key, local resource refs, resolved model connection refs, and launch snapshots.
...
m as live schema documentation targets.
