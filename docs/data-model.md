...
| `run_steps`                        | Persisted workflow step status, copied rerun context, graph metadata, typed tool-failure taxonomy, bounded retry metadata, errors, and timestamps.                                                                                                                                                                                      |
...
- Deleting a schedule deletes its schedule-owned fire rows, stops future automation, preserves existing run history, and keeps direct run artifacts readable through run-owned `scheduleProvenance`. Rerun descendants stay governed by run lineage rather than schedule ownership.
