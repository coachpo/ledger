...
> Status: Live data-model reference for branch `main` at `f9ae90d`.
...
| `run_workflow_package_snapshots` | one immutable executable snapshot per workflow-package run, keyed by `run_id`, with copied package identity, nullable historical `workflow_package_status`, workflow identity, hashes, manifest/export material, compiled plan, local refs, launch inputs, resolved model-connection effective runtime profiles, and preflight summary |
| `model_connections` | global saved provider/model endpoint config, selected `protocol_profile`, capability-status JSONB, output/tool/reasoning/streaming policy fields, probe cache metadata, encrypted API keys, reachability-test metadata, and archive state |
...
- Model-connection secrets remain encrypted at rest and masked in reads/errors. Workflow package manifests store model connection keys as live bindings, not provider credentials. `protocol_profile` is constrained to `openai_chat_completions` or `openai_responses`; `capabilities` is JSONB keyed by public capability names with `supported`, `unsupported`, `unknown`, or `notApplicable` statuses. Connection saves invalidate probe cache state, while reachability-test metadata remains separate from capability probes.
- Model-connection policy columns store output strategy, parallel tool-call, reasoning, and streaming defaults. The current default policies prefer strict schema output, serialize parallel tool calls, allow reasoning hints, allow streaming, and use a 900 second probe cache TTL unless a saved connection overrides them.
- Runs copy the effective non-secret model runtime profile into package provenance at launch. The copied profile includes protocol, model id, sanitized endpoint identity, capabilities, policies, probe-cache TTL, timeout, and `hasApiKey`, while raw key material stays in the live Model Connection secret payload.
