...
| `agent_memory_entries`             | Canonical platform-core memory entries keyed by opaque `memory_id`, with package-qualified scope, kind, subject refs, lifecycle status, content hash, idempotency key, and trusted workflow or operator provenance.                                                                                                                        |
...
- Runtime memory writes canonicalize package, workflow, agent, and namespace scopes before storage. Namespace `scope_key` values use `{ownerPackageKey}/{namespaceKey}`. Shared-scope revision mutations use row-level conflict control and return retryable `memory_revision_conflict` errors instead of allowing competing winners.
- Memory namespace ownership and grants are manifest/runtime data, not separate global user tables. Runtime lookup and scoped API paths authorize against package declarations, grant subjects, and action lists before returning or mutating namespace-scoped rows.
- Admin memory writes use the same canonical entry, revision, and event tables with explicit scope, status, and operator provenance. Operator-created rows use actor `local-instance-operator`, channel `memory_admin`, and append-only `operator_created`, `operator_revised`, or `operator_status_changed` events.
- Approved admin entries can become runtime-visible only when future Workflow Package lookup matches their scope and grant rules. Pending and archived admin entries remain persisted and admin-visible without becoming default runtime lookup results.
- `/api/memory` runtime projections and `/api/memory/admin/entries*` admin projections are derived from `agent_memory_entries`, `agent_memory_revisions`, and `run_memory_events`; they do not query or join `reports` as canonical memory storage.
