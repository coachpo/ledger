...
- Shared memory writes canonicalize package, workflow, agent, and namespace scopes before storage. Namespace `scope_key` values use `{ownerPackageKey}/{namespaceKey}`. Shared-scope revision mutations use row-level conflict control and return retryable `memory_revision_conflict` errors instead of allowing competing winners.
- Memory namespace ownership and grants are manifest/runtime data, not separate global user tables. The API and runtime authorize against package declarations, grant subjects, and action lists before returning or mutating namespace-scoped rows.
- `/api/memory` read/action projections are derived from `agent_memory_entries`, `agent_memory_revisions`, and `run_memory_events`; they do not query or join `reports` as canonical memory storage.
