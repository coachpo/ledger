# TradingAgents-Informed Ledger Upgrade Design

> Status: Research and design note for branch `main` at `596ee84`. Merges the TradingAgents/Ledger workflow comparison with the forward-looking upgrade design; live code remains the source of truth.

## Intent

Ledger should absorb useful TradingAgents capability patterns without adopting the TradingAgents runtime model. Ledger can approximate TradingAgents best as an auditable advisory/research Workflow Package, not as an exact LangGraph clone.
...
TradingAgents source review used DeepWiki plus GitHub source evidence from TradingAgents HEAD `a5cb7cbd61d217fb0bc43f017392a861257afe6a`.

## Settled Scope

In scope:

- Native Ledger data/news and social sentiment runtime tools, migrated or refactored from useful TradingAgents collectors where appropriate.
...
| Area | TradingAgents | TradingAgents implementation | Ledger equivalent | Match | Winner |
|---|---|---|---|---|---|
| Runtime execution | LangGraph node execution with checkpoint resume | `TradingAgentsGraph` compiles a LangGraph `StateGraph`, can use per-ticker SQLite checkpointing, runs with a ticker/date `thread_id`, and clears checkpoints after success. | Persisted Runs with steps, invocations, rerun, and step replay; recovery semantics are similar but not identical. | Medium-High | Ledger, for durable audit/replay design; TradingAgents remains stronger for native graph checkpoint semantics. |
| Memory | Markdown decision log with automatic return/reflection updates and future prompt context | `TradingMemoryLog.store_decision()` appends pending markdown decisions; later runs resolve pending entries with raw/alpha returns, call `Reflector`, and inject past context into the Portfolio Manager prompt. | Report-backed memory via `ledger.reports.lookup` and `ledger.reports.write`; memory is durable, but return/reflection flow is a different report-backed model. | Medium | TradingAgents, for purpose-built trading reflection; Ledger has stronger persistence but less automatic feedback-loop design. |
| Analyst phase | Current source chains analysts sequentially | `GraphSetup` wires selected analysts in order, adds tool loops through `ConditionalLogic`, then proceeds through bull/bear debate, research manager, trader, risk debate, and portfolio manager. | The existing fixture uses analyst `fanout`; Ledger can model sequence if authored that way. | Medium | Tie; TradingAgents has the source-native sequence, while Ledger can implement the same topology when authored explicitly. |
| External data/news research | Source-specific vendor data and news tools with query/fallback behavior, not a generic browser-search runtime | News and data tools route through vendor selection such as `route_to_vendor`; news analysts call `get_news` and `get_global_news` tools for company and macro context. | Private MCP such as Exa MCP in the fixture plus native news/fundamentals/insider tools; coverage depends on configured tools and MCP availability. | Medium | TradingAgents, for built-in market research integrations; Ledger depends on configured MCP/tool availability. |
| **REJECTED - NEVER IMPLEMENT: True graph parity** | LangGraph conditional graph with node-level control flow | TradingAgents is genuinely graph-driven: compiled `StateGraph`, conditional edges, tool loops, debate loops, and portfolio-manager termination. Exact parity means matching LangGraph runtime behavior. | Ledger compiles manifest graph to planned steps; graph metadata is preserved, but execution is intentionally not a LangGraph-equivalent graph runtime. | Rejected | TradingAgents wins exact graph parity, but Ledger must never chase this target because it would change Workflow Package semantics. |
| Trading execution | Trading decision and simulated-exchange framing | `Trader` produces a proposal, risk debate reviews it, `PortfolioManager` emits the final decision, and `SignalProcessor` extracts the final signal; this is not live broker execution. | Ledger package output is advisory; actual trading operations remain separate portfolio/trading APIs. | Medium | TradingAgents, for integrated trading-decision framing; Ledger intentionally separates advisory runs from trade APIs. |
| Social/Reddit/StockTwits | Current TradingAgents sentiment path can use these sources | The sentiment analyst prefetches Yahoo news, StockTwits, and Reddit before the LLM call; StockTwits and Reddit fetchers return plaintext source blocks and degrade gracefully. | Not native in Ledger; requires MCP/new tools or omission. | Low-Medium | TradingAgents, because those sentiment sources are closer to the original implementation path. |

Filtered-out rows include workflow container, agent roles, debate loops, shared state handoff, structured outputs, market/tool categories, auditability, and model configuration because Ledger can reach the same practical runnable outcome through its package, schema, tool, run, and model-connection surfaces.
...
| External data/news research | Mimic vendor-routed source-specific data and news retrieval. | High | Migrate and refactor the useful TradingAgents data/news collection functions into native Ledger runtime tools, then expose them through capability profiles. | Implement as Ledger tools, never MCPs. MCP remains only a user customization surface for workflow packages. |
| Social/Reddit/StockTwits | Mimic the prefetch-and-summarize sentiment approach. | Medium-High | Migrate and refactor the useful StockTwits/Reddit-style collectors into native Ledger sentiment tools that return normalized source blocks and source metrics for agents to summarize. | Provider access, rate limits, and source reliability are the main constraints. Implement as generic Ledger tools, never TradingAgents-specific MCP presets. |
| Trading execution | Mimic proposal-to-decision-to-signal flow, not live execution. | Not needed | Output only the proposed decision and advice from the workflow package. | This application does not need trading execution; keep the result advisory-only and do not create trade-operation drafts from runs. |
| **REJECTED - NEVER IMPLEMENT: True graph parity** | Do not migrate or mimic exact compiled LangGraph runtime semantics. | Rejected | Implement nothing for exact parity; document it as a non-goal and use only current package graph metadata, rerun, and step replay. | Exact node/edge/checkpoint parity would turn Workflow Packages into a LangGraph clone and violate Ledger's universal workflow-platform design. Only approximate outcomes with current package semantics. |

Best implementation targets are memory/reflection automation, native Ledger data/news and social sentiment tools, and a canonical TradingAgents-style package template. The permanently rejected target is exact graph parity; trading execution stays advisory-only.

## Upgrade Tracks

### Native Data And Social Tools

TradingAgents already contains practical data collection functions for vendor-routed news and prefetch-style social sentiment. Ledger should migrate the useful ideas into native runtime tools, not MCP presets.

Design:

- Keep native tool metadata under `/api/tools` and package grants under capability profiles.
- Preserve stable existing tool keys such as `ledger.news.lookup`; add new keys only for new contracts such as normalized social sentiment lookup.
...
- Extend `ledger.news.lookup` for source-specific company and macro news retrieval.
- Add `ledger.social_sentiment.lookup` for Reddit/StockTwits-like source blocks and metrics.
- Keep `ledger.fundamentals.lookup` and `ledger.insider_data.lookup` as financial context tools.

### Memory Follow-Up Automation

Ledger already has `MemoryService`, `ReturnResolutionService`, `ReflectionService`, and `MemoryContextService`. The upgrade should connect them with explicit post-run automation, not hidden workflow loops.

Design:

- Memory writes stay report-backed through `ledger.reports.write`.
...
Use `kind: http` as the public manifest contract. Reserve a broader internal `ExecutionPlanOperation` abstraction for future non-agent families if Ledger later adds `delay`, `approval`, `transform`, or other operation kinds.
...
This is feasible and aligned with Ledger if implemented as additive platform capability:
...
The clean design is still worth it. HTTP operations give workflows normal non-agent actions such as webhooks, notifications, HTTP callbacks, and deterministic API handoffs without turning Ledger into a general code runner or a LangGraph clone.

## Design Decision Summary

- Implement data/news/social collection as native Ledger tools, never MCPs.
