...
Phase-1 Digital Oracle absorption ships as three finance-owned runtime tools that packages can grant through local capability profiles: `signaldeck.prediction_markets.lookup`, `signaldeck.sec_filings.lookup`, and `signaldeck.market_sentiment.lookup`. The Digital Oracle research methodology is package-local prompt policy in an agent `systemPrompt`, not a global skill surface; the final proven demo artifact path is `demo/digital_oracle_researcher.yaml`.
...
- Unscoped global memory browsing, public memory CRUD, exact-id runtime memory get, vector search, embeddings, or memory chunk-table workflows in the current phase.
...
- Historical agent-memory reports remain report-domain records, while canonical memory writes, lookup, `/api/memory`, and `/memory` use platform-core memory tools, tables, scopes, and grant checks.
...
- Model Connections remain global live bindings with backend-owned compatibility truth, global Tools remain read-only metadata, finance-owned Digital Oracle phase-1 tools remain package-authoring choices behind `signaldeck.finance`, platform-core memory tools and `/api/memory` remain separate from finance, and package-private resources stay inside package versions.
