...
9. Memory: platform-core scoped runtime `/api/memory` workflows plus trusted local operator `/api/memory/admin/entries*` and `/memory` admin management over canonical workflow memory across packages. Runtime `signaldeck.core.memory.lookup/write` and scoped `/api/memory` remain package/run/agent/scope-bound and do not expose finance report history as memory.
