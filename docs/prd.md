...
8. Runs: global run list/detail, dedicated package launch, backend-owned progress and queue state, run-owned package snapshots, operation invocation evidence, root-parameter reruns, typed tool-failure taxonomy, and bounded model-feedback retry evidence.
