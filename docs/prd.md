...
- Model Connections remain global live bindings with backend-owned compatibility truth, global Tools remain read-only metadata, platform-core memory tools and `/api/memory` remain separate from finance, and package-private resources stay inside package versions.
