...
| `api_router` | `backend/app/api/router.py` | mounts all `/api/v1` routers, including templates and reports |
...
| `TemplateCompilerService` | `backend/app/services/template_compiler_service.py` | resolves `{{portfolios...}}` and `{{reports...}}` placeholders against live data |
...
| `router` | `frontend/src/routes.ts` | flat route table for dashboard, portfolios, templates, and reports |
