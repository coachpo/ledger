...
- `backend/app/services/stock_analysis/AGENTS.md` — context building, execution modes, parser rules
...
- `frontend/src/components/stock-analysis/AGENTS.md` — run builder, preview, conversation, run-status flows
...
| Frontend app shell | `frontend/src/App.tsx`, `frontend/src/routes.ts`, `frontend/src/components/layout.tsx`, `frontend/AGENTS.md` | query client, flat router shell, and a sidebar with seven destinations |
...
| Stock-analysis UI | `frontend/src/components/stock-analysis/AGENTS.md` | run builder, conversation picker, prompt preview, status polling |
...
| `StockAnalysisService` | `backend/app/services/stock_analysis/service.py` | settings, conversations, runs, responses, versions |
...
| `router` | `frontend/src/routes.ts` | flat route table for dashboard, portfolios, LLM inputs, run builder, responses |
...
| `useStockAnalysis*` hooks | `frontend/src/hooks/use-stock-analysis.ts` | settings, conversations, runs, responses, preview, execute |
| `PortfolioDetailPage` | `frontend/src/components/portfolios/portfolio-detail-page.tsx` | balances/positions/trades workspace |
| `RunBuilderPage` | `frontend/src/components/stock-analysis/run-builder-page.tsx` | portfolio selection + conversation/run orchestration |
...
- Global stock-analysis resources live under `/stock-analysis/*` (`llm-configs`, `prompt-templates`, `snippets`); portfolio settings, conversations, runs, responses, and versions stay under `/portfolios/{id}/stock-analysis/*`.
- Prompt execution supports `single_prompt` (request/response history only) and `two_step_workflow` (versioned fresh vs compare/decide/reflect flow).
