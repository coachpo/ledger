...
- Reports are point-in-time markdown snapshots keyed by unique `slug`; canonical `source` origins are `compiled`, `uploaded`, `external`, and `agent`. `external` stays limited to true external user/API-created reports. Historical agent-memory reports are report-domain history only; workflow runtime memory writes and lookup are declared through Workflow Package `spec.memory` and handled by platform-core workflow memory middleware, policy, and review surfaces.
