...
## OVERVIEW
`backend/tests/` is the behavioral spec for the shipped backend surface: preserved template/report APIs behind Finance Workspace gates; Finance and Digital Oracle runtime tools; extension registry/state behavior; current agent-platform APIs; database bootstrap; and run rerun semantics. Tests run against isolated PostgreSQL databases and a real FastAPI app instance.
...
| API regression coverage | `test_api.py` | preserved templates, reports, extension gating, platform validation, and neutral stub workflow coverage |
