...
## OVERVIEW
`backend/tests/` is the behavioral spec for the shipped backend surface: preserved portfolio, template, report, trading, and market-data APIs behind `signaldeck.finance`; extension registry/state behavior; social sentiment/runtime tools; current agent-platform APIs; startup schema upgrades; run rerun/fork semantics; and destructive cutover assertions. Tests run against isolated PostgreSQL databases and a real FastAPI app instance.
...
| Agent-platform run coverage | `test_workflow_package_runtime_api.py`, `test_workflow_package_run_contracts.py`, `test_run_operation_invocations.py` | package execution, run detail/list, model-binding provenance, reruns, invocation-input forks, HTTP/tool operations, and legacy step replay read-lineage coverage |
