...
def test_model_connection_upgrade_deletes_legacy_deterministic_smoke_rows_and_drops_kind(  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
    database_url: str,
) -> None:
    legacy_smoke_url = "https://signaldeck-deterministic-model.local/v1"
    engine = create_engine(database_url, future=True)
            ...
            connection.execute(
                text(
                    ...
                    CREATE TABLE model_connections (
                        ...
                        connection_kind /* OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP */ VARCHAR(40) NOT NULL DEFAULT 'provider',
                        ...
                        CONSTRAINT ck_model_connections_connection_kind /* OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP */ CHECK (
                            connection_kind /* OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP */ IN ('provider', 'deterministic_smoke') -- OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
                        ),
            ...
            connection.execute(
                text(
                    ...
                    INSERT INTO model_connections (
                        status, connection_kind /* OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP */, name, description, base_url, model_id,
                        reasoning_effort, api_style, timeout_seconds, secret_payload,
                        created_at, updated_at
                    ) VALUES (
                        'active', :connection_kind /* OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP */, :name, '', :base_url, :model_id,
                        'medium', 'responses', 60, '{}'::jsonb, NOW(), NOW()
                    )
                    """
                ),
                [
                    {
                        "base_url": legacy_smoke_url,
                        "connection_kind": "deterministic_smoke",  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
                        "model_id": "openai:gpt-5.4-mini",
                        "name": "Deterministic Smoke",  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "connection_kind": "provider",  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
                        "model_id": "openai:gpt-5.4-mini",
                        "name": "Provider On Legacy Smoke Url",
                    },
                ],
            )
...
def test_model_connection_upgrade_scrubs_run_snapshot_kind_keys(database_url: str) -> None:
    init_db(database_url)
    engine = create_engine(database_url, future=True)
    ...
    try:
        with engine.begin() as connection:
            ...
            package_id = _insert_current_package(connection, "snapshot_cleanup_package")
            run_id = connection.execute(
                text(
                    """
                    INSERT INTO runs (
                        ...
                        'workflowPackage', :package_id, 'snapshot_cleanup_package', 1,
                        :package_id, 'snapshot_cleanup_package', 'daily', '{}'::jsonb, 'queued'
                    ) RETURNING id
                    """
                ),
                {"package_id": package_id},
        ...
        with engine.connect() as connection:
            resolved_connections = connection.execute(
                text(
                    """
                    SELECT resolved_model_connections
        ...
        assert resolved_connections == [
            {"key": "live_model", "modelId": "openai:gpt-5.4-mini"},
