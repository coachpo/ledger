...
## OVERVIEW
`app/models/` defines SQLAlchemy ORM entities, table names, constraints, indexes, cache tables, and relationships for the preserved product data model and current agent-platform tables. Models stay persistence-focused and should not contain service-layer business rules.
...
- Unique constraints enforce business rules such as unique template names, schedule fire keys, versioned platform keys, and quote-cache lookup keys.
