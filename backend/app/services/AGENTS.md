...
| Memory workflows | `memory_service.py`, `memory_context_service.py`, `memory_store.py`, `memory_follow_up_service.py`, `memory_report_service.py` | core memory DTO lifecycle, Postgres persistence, follow-up evaluation, prompt snippets, run evidence, and historical report-domain readers |
...
| Extension state/filtering | `extension_service.py`, `extension_dependency_service.py`, `extension_gate.py`, `quote_provider_bridge.py` | slim statically resident extension state, surface gating, ToolCatalog/runtime registry filtering, dependency-only run extension records, and provider/runtime bridges |
| Model gateway / connection truth | `model_gateway.py`, `model_gateway_dto.py`, `model_gateway_openai.py`, `model_gateway_openai_responses.py`, `model_gateway_output_validation.py`, `model_gateway_policy_strategy.py`, `model_gateway_tool_strategy.py`, `model_gateway_tool_retry.py`, `model_connection_service.py`, `model_connection_probe_service.py`, `model_connection_compatibility.py` | persisted model-connection state, capability/probe truth, official-SDK execution, tool/schema validation, responses/policy adapters, and protocol DTOs |
...
| Scheduled task services | `workflow_package_schedule_service.py`, `workflow_package_schedule_materializer.py` | structured recurrence, IANA timezone occurrence math, scheduled input rendering, idempotent fires, archive/read-only rules, due materialization into queued runs |
| Scheduled task services | `workflow_package_schedule_service.py`, `workflow_package_schedule_materializer.py` | structured recurrence, IANA timezone occurrence math, scheduled input rendering, idempotent fires, archive/read-only rules, due materialization into queued runs |
...
- Workflow package services keep one mutable current package artifact, validate typed contracts before save, keep private MCP `env`, `headers`, and `query` values inline through import/export, and keep runtime-input registry/history plus run persistence detailed enough for snapshot provenance, rerun/fork lineage, and the run monitor.
- Scheduled task services keep recurrence and fire semantics backend-owned: structured recurrence only, IANA timezone conversion, placeholder allowlist, preview-before-save validation, idempotent manual fires, archived schedules read-only, and scheduled/manual fires materialized as ordinary queued package runs.
...
- `backend/app/services/providers/` is effectively reserved right now; keep provider/runtime seams in the named bridge and gateway services until a real subpackage boundary exists.
