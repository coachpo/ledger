...
- Scheduled task services keep recurrence and fire semantics backend-owned: structured recurrence only, IANA timezone conversion, placeholder allowlist, preview-before-save validation, idempotent manual fires, direct schedule-owned run and fire cleanup on delete, and scheduled or manual fires materialized as ordinary queued package runs.
