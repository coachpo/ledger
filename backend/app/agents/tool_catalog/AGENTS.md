...
## OVERVIEW
`app/agents/tool_catalog/` owns read-only server-declared tool metadata and package capability-profile validation. It resolves known tool keys from installed extensions and exposes the slim `/api/tools` contract through backend services/routes.
...
| Task | Location | Notes |
|---|---|---|
| Catalog behavior | `__init__.py` | `ToolCatalog`, duplicate/unknown key validation |
...
- Do not expose module names, owner extension keys, scaffold metadata, or plugin-manifest fields through `/api/tools`.
- Do not validate package `toolKeys` against ad hoc tool lists outside the installed extension catalog.
