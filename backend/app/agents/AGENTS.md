...
- Server-declared finance tool keys use the `signaldeck.finance.*` namespace for market quote/history/OHLCV, indicators, fundamentals, news, social sentiment, insider data, positions, and report lookup. Server-declared Digital Oracle tool keys use `signaldeck.digital_oracle.prediction_markets.lookup`, `signaldeck.digital_oracle.sec_filings.lookup`, and `signaldeck.digital_oracle.market_sentiment.lookup` with mechanical OpenAI function names derived from the canonical keys. Core memory tools are platform-owned.
