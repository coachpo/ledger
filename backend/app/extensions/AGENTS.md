...
## OVERVIEW
`app/extensions/` owns first-party extension registration and extension-owned composition roots. The current statically resident bundled extensions are Finance Workspace and Digital Oracle Runtime.

`signaldeck.finance` owns preserved finance `/api/v1` template/report routers, finance provider factories, finance grants/dependency records, finance runtime tool specs/executors, split runtime modules for market data/reports, and report lookup. Its public runtime tool keys are `signaldeck.finance.market_data.quote_lookup`, `signaldeck.finance.market_data.history_lookup`, `signaldeck.finance.market_data.ohlcv_lookup`, `signaldeck.finance.indicators.lookup`, `signaldeck.finance.fundamentals.lookup`, `signaldeck.finance.news.lookup`, `signaldeck.finance.social_sentiment.lookup`, `signaldeck.finance.insider_data.lookup`, and `signaldeck.finance.reports.lookup`.
...
Extension model: this folder owns statically resident extension registration and extension-owned composition roots for code shipped with SignalDeck Core.
...
## CHILD DOCS
- `signaldeck_finance/AGENTS.md` — statically resident `signaldeck.finance` ownership, route registrations, provider factories, grants, split runtime modules, report lookup
- `signaldeck_digital_oracle/AGENTS.md` — statically resident `signaldeck.digital_oracle` tool-only runtime ownership

## WHERE TO LOOK
| Task | Location | Notes |
|---|---|---|
| Bundled registry | `registry.py` | installed extension list and static contract helpers |
...
## CONVENTIONS
- Extension definitions are private registry wiring; behavior is supplied through explicit `EXTENSION` contracts.
...
- There is no runtime extension state API. Add or remove installed extensions by editing `INSTALLED_EXTENSIONS`.
...
- Do not add plugin marketplace, install, remove, enable, or disable semantics to statically resident extensions.
- Do not expose registry/scaffold metadata through `/api/tools`, OpenAPI, run dependency records, or docs.
