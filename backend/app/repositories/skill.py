...
class SkillRepository(BaseRepository[Skill]):
    ...
    def list_latest_versions(self, *, status: str | None = None) -> list[Skill]:
    ...
    def list_versions(self, key: str) -> list[Skill]:
        statement = (
            select(self.model)
    ...
    def get_draft_by_key(self, key: str) -> Skill | None:
        statement = select(self.model).where(self.model.key == key, self.model.status == "draft")
        return self._get_by_statement(statement)

    def resolve_version(self, key: str, version: int | None) -> Skill | None:
        if version is None:
