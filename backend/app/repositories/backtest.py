...
_TERMINAL_BACKTEST_STATUSES = ("COMPLETED", "FAILED", "CANCELLED")
...
class BacktestRepository(BaseRepository[Backtest]):
    ...
    def list_non_terminal(
        ...
        *,
        ...
        statement = select(self.model).where(~self.model.status.in_(_TERMINAL_BACKTEST_STATUSES))
        if execution_owner is not None:
            statement = statement.where(self.model.execution_owner == execution_owner)
