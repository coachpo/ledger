...
class RuntimeTraceEventRepository(BaseRepository[RuntimeTraceEvent]):
    model = RuntimeTraceEvent
    ...
    def list_for_run(self, run_id: int) -> list[RuntimeTraceEvent]:
        return self.list_all(run_id=run_id)

    def get_for_run_index(self, run_id: int, event_index: int) -> RuntimeTraceEvent | None:
        statement = select(self.model).where(
            self.model.run_id == run_id,
            self.model.event_index == event_index,
        )
        return self._get_by_statement(statement)

    def get_latest_for_run(self, run_id: int) -> RuntimeTraceEvent | None:
        statement = (
            select(self.model)
            .where(self.model.run_id == run_id)
            .order_by(
                self.model.event_index.desc(), self.model.created_at.desc(), self.model.id.desc()
            )
        )
