...
| Extension-aware providers | `../extensions/registry.py` | resolves the installed execution-provider bundle before execution |
...
- The worker resolves the execution-provider bundle through the installed extension registry at execution time.
...
- Do not resolve finance/provider/runtime ownership directly in the worker when the extension registry owns that boundary.
