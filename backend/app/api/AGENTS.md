...
| Service construction | `dependencies.py` | request-scoped session plus CRUD, CSV import, trading, market-data, template, report, and quote-provider factories |
...
- Do not bypass dependencies when wiring services, template helpers, or quote providers.
...
## NOTES
- `router.py` mounts all live routers under `/api/v1`: backtests, backtest callbacks, portfolios, balances, positions, trading operations, market data, templates, and reports.
- `dependencies.py` constructs services with a shared request `Session` and wires both `BacktestService` and `BacktestCycleService` alongside `CsvImportService`, `TradingOperationService`, `MarketDataService`, `TextTemplateService`, `TemplateCompilerService`, `ReportService`, and `YahooFinanceQuoteProvider` into the live API.
