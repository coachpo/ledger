...
## OVERVIEW
`app/api/` owns FastAPI `APIRouter` modules, request/response contracts, dependency wiring, and translation from service-layer errors into HTTP responses. Routers stay thin and delegate business rules to services. The live app mounts preserved `/api/v1` finance routes through installed extension contracts plus current `/api/*` platform routes for Workflow Packages, Scheduled Tasks, Model Connections, Tools, and Runs.

Extension model: statically resident `INSTALLED_EXTENSIONS` contracts.
...
| Task | Location | Notes |
|---|---|---|
| Router composition | `router.py`, `platform_router.py`, `../main.py` | `router.py` includes extension `/api/v1` routers from `INSTALLED_EXTENSIONS`, `platform_router.py` composes `/api/*`, and `main.py` mounts both |
| Service construction | `dependencies.py` | request-scoped session plus finance, model-connection, tool-catalog, workflow-package, schedule, and run service factories |
...
| Platform routes | `workflow_packages.py`, `schedules.py`, `model_connections.py`, `tools.py`, `runs.py` | live `/api/*` routes for Workflow Packages, Scheduled Tasks, Model Connections, Tools, and Runs, including rerun endpoints under Runs |
...
- `main.py` mounts `/api/v1` via `router.py` and `/api/*` via `platform_router.py`; `/api/v1` route registrations come from `INSTALLED_EXTENSIONS`.
...
- Workflow Package routes are canonical for platform authoring. The mounted platform routers are Workflow Packages, Scheduled Tasks, Model Connections, Tools, and Runs. Package manifests keep agents, output schemas, capability profiles, private MCP configs, and workflow graphs package-private.
...
uv run mypy app
uv run pytest tests/test_api.py tests/test_extension_contract.py tests/test_tool_catalog_api.py tests/test_workflow_package_openapi.py tests/test_workflow_package_runtime_api.py
```

## NOTES
- `router.py` mounts the live `/api/v1` routers declared by installed extensions.
- `platform_router.py` mounts the live `/api/*` routers for workflow packages, schedules, model connections, tools, and runs.
- `dependencies.py` constructs services with a shared request `Session` and wires finance services, model-connection service, tool catalog, workflow package services, schedule services, and run service into the live API.
