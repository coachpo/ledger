    ...
    if "model_connections" in table_names:
        ...
        for constraint in inspector.get_check_constraints("model_connections"):
            ...
            if not constraint_name:
                continue
            if constraint_name == _MODEL_CONNECTION_KIND_CHECK or "connection_kind" in str(  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
                constraint.get("sqltext") or ""
            ):
                model_connection_kind_checks.add(str(constraint_name))  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP

    with engine.begin() as connection:
        _scrub_legacy_model_connection_kind_snapshot_json(connection, table_names)  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
        if "model_connections" not in table_names:
            return
        if "connection_kind" in model_connection_columns:  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
            _ = connection.execute(
                text(
                    """
                ...
                {"legacy_connection_kind": "deterministic_smoke"},  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
            )
        for constraint_name in model_connection_kind_checks:  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
...
def upgrade_legacy_schema(engine: Engine) -> None:
    ...
    _remove_legacy_model_connection_kind_support(engine, table_names)  # OMO_ALLOW_LEGACY_MODEL_CONNECTION_CLEANUP
