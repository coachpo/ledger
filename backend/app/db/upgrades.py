...
_PRESET_PACKAGE_MANIFEST_HASH = "048223c1c14507f8e63103fbe59136dce116a08bfedcf7650471cd8406ce4dd0"
_PRESET_PACKAGE_COMPILED_HASH = "191fcf5aa8c22567605517d40bf20a5bdce2d6cba778e073a0b522b0f5498a7f"
...
_DIGITAL_ORACLE_PRESET_PACKAGE_COMPILED_HASH = (
    "a21394e1cf59d60284d0b24c71444f0fc9d62273c97c3694b9bade4f5296f6ad"
)
...
_OLD_MEMORY_TABLE_RENAMES = {
    "agent_memory_entries": "legacy_agent_memory_entries",
    "agent_memory_revisions": "legacy_agent_memory_revisions",
    "run_memory_events": "legacy_run_memory_events",
}
_OLD_MEMORY_TABLE_DROP_ORDER = (
    "run_memory_events",
...
def _isolate_legacy_memory_tables(engine: Engine, table_names: set[str]) -> None:
    with engine.begin() as connection:
        for table_name in _REMOVED_CORE_MEMORY_TABLE_DROP_ORDER:
            connection.exec_driver_sql(f'DROP TABLE IF EXISTS "{table_name}" CASCADE')
            table_names.discard(table_name)

        for old_table_name in _OLD_MEMORY_TABLE_DROP_ORDER:
            if old_table_name not in table_names:
                continue
            legacy_table_name = _OLD_MEMORY_TABLE_RENAMES[old_table_name]
            if not _table_has_rows(connection, old_table_name):
                connection.exec_driver_sql(f'DROP TABLE IF EXISTS "{old_table_name}" CASCADE')
                table_names.discard(old_table_name)
                continue
            if legacy_table_name in table_names:
                raise RuntimeError(
                    f"Cannot isolate non-empty {old_table_name}: {legacy_table_name} already exists"
                )
            connection.exec_driver_sql(
                f'ALTER TABLE "{old_table_name}" RENAME TO "{legacy_table_name}"'
            )
            table_names.discard(old_table_name)
            table_names.add(legacy_table_name)
...
def upgrade_legacy_schema(engine: Engine) -> None:
    ...
    _isolate_legacy_memory_tables(engine, table_names)
