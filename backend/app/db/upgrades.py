...
_PRESET_PACKAGE_MANIFEST_HASH = "6514cf19fb616ed4c46fb858cf2df0132e5959914b6d5b8091564e43c983e9c6"
_PRESET_PACKAGE_COMPILED_HASH = "5f78dfec866f75231bff93a252ea949438b2be107ad88190666501c27da89ac3"
_DIGITAL_ORACLE_PRESET_PACKAGE_MANIFEST_HASH = (
    "8b9bd7d622d63848e65ec5cc66311f51cee547916ea1e4cebb9679028a554c72"
)
_DIGITAL_ORACLE_PRESET_PACKAGE_COMPILED_HASH = (
    "bacbc9d29f2daa4ace52a8ab243322b0672b71b7a80afa36793107d7e55c2a68"
)
...
_REMOVED_CORE_MEMORY_TABLE_DROP_ORDER = (
...
_FINANCE_MEMORY_METADATA_TABLE_NAMES = {"signaldeck_finance_memory_metadata"}
_FINANCE_MEMORY_METADATA_TABLE_STATEMENTS: tuple[str, ...] = (
    """
    CREATE TABLE IF NOT EXISTS signaldeck_finance_memory_metadata (
        memory_id VARCHAR(160) PRIMARY KEY
            REFERENCES agent_memory_entries(memory_id) ON DELETE CASCADE,
        ...
        CONSTRAINT ck_signaldeck_finance_memory_metadata_action CHECK (
            action IN ('buy', 'hold', 'sell')
        ),
        CONSTRAINT ck_signaldeck_finance_memory_metadata_horizon_days_positive CHECK (
...
def _ensure_runtime_input_registry_table(engine: Engine, table_names: set[str]) -> None:
    ...
    registry_columns = {
        column["name"] for column in inspector.get_columns(_RUNTIME_INPUT_REGISTRY_TABLE_NAME)
    }

    with engine.begin() as connection:
        if {"owner_type", "owner_id"} <= registry_columns:
            ...
            if invalid_owner_count:
                raise RuntimeError(
                    "workflow_package_runtime_input_entries contains invalid owner scope; "
                    "expected only local_user/default legacy rows before owner cutoff"
                )
        ...
        connection.exec_driver_sql(
            "UPDATE workflow_package_runtime_input_entries "
            "SET slot = 'preset' "
            "WHERE slot = 'personal'"
        )
        ...
        for column_name in ("owner_type", "owner_id"):
            if column_name in registry_columns:
                connection.exec_driver_sql(
...
def _ensure_core_memory_tables(engine: Engine, table_names: set[str]) -> None:
    with engine.begin() as connection:
        for table_name in _REMOVED_CORE_MEMORY_TABLE_DROP_ORDER:
            connection.exec_driver_sql(f'DROP TABLE IF EXISTS "{table_name}" CASCADE')
            table_names.discard(table_name)
        ...
        for index_name in (
            ...
            "ix_agent_memory_revisions_attributes_gin",
        ):
            connection.exec_driver_sql(f"DROP INDEX IF EXISTS {index_name}")
        _drop_constraint_if_exists(
            connection,
            "agent_memory_entries",
            "ck_agent_memory_entries_attributes",
        )
        _drop_constraint_if_exists(
            connection,
            "agent_memory_revisions",
            "ck_agent_memory_revisions_attributes",
        )
        connection.exec_driver_sql(
            "ALTER TABLE agent_memory_entries DROP COLUMN IF EXISTS attributes CASCADE"
        )
        connection.exec_driver_sql(
            "ALTER TABLE agent_memory_revisions DROP COLUMN IF EXISTS attributes CASCADE"
        )
        for index_name in (
        ...
        for statement in _FINANCE_MEMORY_METADATA_TABLE_STATEMENTS:
            connection.exec_driver_sql(statement)
    table_names.update(_CORE_MEMORY_TABLE_NAMES)
    table_names.update(_FINANCE_MEMORY_METADATA_TABLE_NAMES)
...
def upgrade_legacy_schema(engine: Engine) -> None:
    ...
    _drop_tables(engine, table_names, _RETIRED_GLOBAL_AUTHORING_TABLES)
    _ensure_hard_delete_lifecycle_schema(engine, table_names)
