...
- `docs/` — concise product and data-model docs plus retained extension guidance and migration plans
...
The example compose uses `latest` image tags for readability; replace them with an exact release tag for production. It also requires `SIGNALDECK_API_TOKEN`; deployments that rely only on an authenticated reverse proxy should remove that environment entry deliberately.
